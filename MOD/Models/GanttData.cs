        ...
        public List<GanttDataSource> BaselineData(int Id)
        {
            ...
            if (_isUserExdfxists != null)
            {
                foreach (tbl_trn_tryProject1 item in _isUserExdfxists)
                {
                    try
                    {
                        list = new GanttDataSource
                        {
                            //TaskId = Convert.ToInt32(item.TryPojectID),
                            //TaskName = item.TaskDescription,

                            //BaselineStartDate = new DateTime(2018, 03, 11, 10, 0, 0),//Convert.ToDateTime(item.planned_start_date),
                            //BaselineEndDate = new DateTime(2018, 03, 21, 10, 0, 0),   //                        Convert.ToDateTime(item.planned_end_date),
                            //Predecessor = item.Dependencies.ToString(),
                            //Predecessor1 = Convert.ToInt32(item.Dependencies),

                            TaskId = Convert.ToInt32(item.TryPojectID),
                            TaskName = item.taskdescription,
                            BaselineStartDate = Convert.ToDateTime(item.planned_start_date),
                            BaselineEndDate = Convert.ToDateTime(item.planned_end_date),
                            StartDate = item.startdate,
                            EndDate = item.enddate,
                            Predecessor1 = Convert.ToInt32(item.Dependencies),
                            Progress = Convert.ToInt32(item.progress),
                    ...
                    //  var _isupdateStatusExdfxists = _entities.tbl_trn_UpdateStausDummy.ToList();
                 

                    GanttDataSourceCollection.Add(list);
                }
            }
            return GanttDataSourceCollection;
        }
        public List<GanttDataSource> ProjectNewData(int Id)
        {
            List<GanttDataSource> GanttDataSourceCollection = new List<GanttDataSource>();
